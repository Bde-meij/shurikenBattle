...
var playerShadows: Actor[] = addPlayerShadows(
  players[0],
  players[1],
  players[2],
  players[3]
...
function startGameLoop() {
  game.on("postupdate", () => {
    for (var i = 0; i < 4; i++) checkPlayerCollision(i);
    checkBossCollission();
    updateHack++;
    if (updateHack > 10000) updateHack = 0;
    if (updateHack % 4 == 0) {
...
function checkPlayerCollision(i: number) {
  players[i].on("collisionstart", () => {
    if (bounced == false) {
      bounced = true;
      shurikenSpeed = bounce(
        shurikenSpeed,
        [players[i].pos.x, players[i].pos.y],
        [bigBoss.pos.x, bigBoss.pos.y]
      );
    }
  });
}
...
function addAssets() {
  ...
  game.add(shuriken);
  for (var i = 11; i >= 0; i--) game.add(playerShadows[i]);
  game.add(players[0]);
  game.add(players[1]);
  game.add(players[2]);
  game.add(players[3]);
...
function removeAssets() {
  ...
  for (var i = 11; i >= 0; i--) game.remove(playerShadows[i]);
...
function moveShuriken() {
  ...
  if (rotating == true) {
    shurikenSpeed = rotateVec(shurikenSpeed, dirMod);
  }
}
...
function dealDamage() {
  ...
  damageCounter += 1;
  if (damageCounter > 3) lifebar.color = Color.Red;
  else if (damageCounter > 1) lifebar.color = Color.Orange;
  game.currentScene.camera.shake(halfSize * 0.05, halfSize * 0.05, 350);
}

function checkBossCollission() {
  var distance = pointDiff(
    [bigBoss.pos.x, bigBoss.pos.y],
    [shuriken.pos.x, shuriken.pos.y]
  );
