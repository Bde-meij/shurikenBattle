...
export function playerMovement(player: Actor, key: Keys) {
  if (player.pos.y > halfSize) mod = 1;
  else mod = -1;
  ...
  if (player.pos.x > halfSize + radius / 2) {
    if (key == Keys.A) player.pos.y -= Math.abs(dirSpeed);
    if (key == Keys.D) player.pos.y += Math.abs(dirSpeed);
    player.pos.x =
      halfSize +
    ...
    dirSpeed =
      playerSpeed * (Math.abs(player.pos.y - halfSize) / halfSize) * mod;
    if (key == Keys.A) player.pos.x += dirSpeed;
    if (key == Keys.D) player.pos.x -= dirSpeed;
...
export function addPlayerShadows(
  playerOne: Actor,
  playerTwo: Actor,
  playerThree: Actor,
  playerFour: Actor
